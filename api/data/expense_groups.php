...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    switch ($target) {
        case 'sub-parish':
            $sql = "SELECT seg.expense_group_id, seg.expense_group_name, seg.expense_group_identifier, seg.account_id, sp.sub_parish_name, b.account_name
                    FROM sub_parish_expense_groups seg
        ...
        case 'community':
            $sql = "SELECT ceg.expense_group_id, ceg.expense_group_name, ceg.expense_group_identifier, ceg.account_id, c.community_name, sp.sub_parish_name, b.account_name
                    FROM community_expense_groups ceg
        ...
        case 'group':
            $sql = "SELECT geg.expense_group_id, geg.expense_group_name, geg.expense_group_identifier, geg.account_id, g.group_name, b.account_name
                    FROM group_expense_groups geg
        ...
        case 'head-parish':
        default:
            $sql = "SELECT heg.expense_group_id, heg.expense_group_name, heg.expense_group_identifier, heg.account_id, hp.head_parish_name, b.account_name
                    FROM head_parish_expense_groups heg
    ...
    if (!empty($searchQuery)) {
        $sql .= " AND (" . implode(" LIKE '%$searchQuery%' OR ", ['expense_group_name', 'account_name']) . " LIKE '%$searchQuery%')";
    }
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(["success" => false, "message" => "Failed to fetch expense groups data: " . $stmt->error]);
    }
