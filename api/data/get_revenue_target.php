...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Prepare SQL query to fetch revenue target
    $sql = "SELECT revenue_target_amount FROM head_parish_annual_revenue_targets 
            WHERE head_parish_id = ? AND bank_account_id = ?
            AND revenue_target_start_date = ? AND revenue_target_end_date = ?";
    ...
    if ($row = $result->fetch_assoc()) {
        echo json_encode(["success" => true, "data" => ["revenue_target" => $row['revenue_target_amount']]]);
    } else {
        echo json_encode(["success" => false, "message" => "Revenue target not found"]);
    }
