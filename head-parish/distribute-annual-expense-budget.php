...
<head>
  <?php 
    ...
    render_header('Distribute Expense Budget - Kanisa Langu');
  ?>
</head>
  ...
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
      ...
      <div class="body-wrapper">
        ...
        <div class="container-fluid">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title fw-semibold mb-4">Distribute Annual Expense Budget</h5>
                ...
                <div class="tab-pane fade show active" id="head-parish" role="tabpanel">
                  <form id="headParishForm" autocomplete="off">
                    ...
                    <div class="row">
                      ...
                      <div class="col-lg-6">
                        <div class="mb-3">
                          <label for="expenseBudgetHP" class="form-label">Annual Expense Budget</label>
                          <input type="number" class="form-control" id="expenseBudgetHP" name="expense_budget_target_amount" placeholder="Expense Budget" min="0" readonly>
                        </div>
                      </div>
                    </div>
                    <div class="row">
                    ...
                    <button type="submit" class="btn btn-primary">Distribute Expense Budget</button>
                  </form>
                </div>
              </div>
  ...
  $(document).ready(function () {
    ...
    function submitForm(formId, target) {
      const formData = $(formId).serialize() + '&target=' + target;
        $('#responseMessage').html('<div class="response-message"><i class="fas fa-spinner fa-spin icon"></i> Distributing Expense Budget...</div>');
      $.ajax({
        type: 'POST',
        url: '../api/records/distribute_annual_expense_budget',
        data: formData,
        dataType: 'json',
...
// Function to load expense budget when bank account is selected
    $('#accountIdHP').on('change', function () {
        ...
        if (accountId) {
            $.ajax({
                type: 'GET',
                url: `../api/data/get_annual_expense_budget?account_id=${accountId}&start_date=${startDate}&end_date=${endDate}`,
                dataType: 'json',
                success: function (response) {
                    if (response.success) {
                        $('#expenseBudgetHP').val(response.data.expense_budget_target);
                    } else {
                        $('#expenseBudgetHP').val('');
                ...
                error: function () {
                    $('#expenseBudgetHP').val('');
                    alert('Error fetching expense budget.');
                }
            });
        } else {
            $('#expenseBudgetHP').val('');
