...
<div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6"
  ...
  <div class="body-wrapper">
    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-body">
          <h5 class="card-title fw-semibold mb-4">Manage Harambee Excluded Church Members</h5>

          <!-- Search Form -->
          <div class="row mb-4">
            <div class="col-md-8">
              <input type="text" id="searchExclusions" class="form-control" placeholder="Search by name, target, or Harambee">
            </div>
...
<script>
  $(document).ready(function () {
    ...
    function fetchExclusions(page = 1, query = '') {
        ...
        const url = `../api/data/excluded_church_members_from_harambee.php?limit=${limit}&page=${page}&query=${encodeURIComponent(query)}`;
        ...
        if (response.data.length === 0) {
          tbody.append('<tr><td colspan="7" class="text-center">No excluded members found.</td></tr>');
        } else {
          let index = (page - 1) * limit + 1;
           response.data.forEach(item => {
            ...
            <tr data-id="${item.exclusion_id}">
                ...
                <small>Envelope: ${item.member.envelope_number || 'N/A'} | Phone: ${item.member.phone || 'N/A'}</small>
                </td>
                <td>${item.target}</td>
                <td>${item.harambee_name}</td>
                <td>${item.reason}</td>
    ...
    $('#confirmDeleteBtn').click(function () {
      ...
      $.post('../api/delete/delete_harambee_excluded_church_member.php', { exclusion_id: excludeToDelete.id }, function (response) {
