...
<head>
  <?php 
    ...
    render_header('Harambee Contribution Report - Kanisa Langu');
  ?>
</head>
  ...
  <div class="page-wrapper" id="main-wrapper" data-layout="vertical" data-navbarbg="skin6" data-sidebartype="full"
      ...
      <div class="body-wrapper">
        ...
        <div class="container-fluid">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title fw-semibold mb-4">Harambee Contribution Report</h5>

              <!-- Toggle: download xlsx instead of pdf -->
              <div class="mb-3 d-flex align-items-center">
                <div class="form-check form-switch me-2">
                  <input class="form-check-input" type="checkbox" id="downloadXlsxToggle" name="download_xlxs" value="true">
                </div>
                <label for="downloadXlsxToggle" class="form-label mb-0">Download XLSX instead of PDF</label>
              </div>
...
$(document).ready(function () {
    ...
    function submitForm(formId, target) {
        // read toggle state
        const downloadXlsxChecked = $('#downloadXlsxToggle').is(':checked') ? 'true' : 'false';

        // serialize form then append the explicit param as the API expects download_xlxs
        const formData = $(formId).serialize() + '&target=' + target + '&download_xlsx=' + encodeURIComponent(downloadXlsxChecked);

        $.ajax({
            type: 'POST',
            url: '../api/data/download_harambee_report',
            data: formData,
            dataType: 'json',
            success: function (response) {
                ...
                if (response.success) {
                    ...
                    }, 1000); // Wait 1 second before redirecting or reloading
                } else {
  ...
  function loadHarambee(targetId, url) {
    ...
    // Load communities for Head Parish when a Sub Parish is selected
    $('#subParishIdHP').on('change', function () {
      ...
      if (subParishId) {
        $.ajax({
          ...
          error: function (xhr, status, error) {
            console.log('Error loading communities for Head Parish:', error);
          }
        });
      } else {
        $('#communityIdHP').html('<option value="">Select Community</option>');
      }
    });
