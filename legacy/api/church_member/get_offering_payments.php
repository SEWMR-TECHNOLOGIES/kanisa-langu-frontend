...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $service_id = isset($_POST['serviceId']) ? intval($_POST['serviceId']) : null;

    // Validate the required parameters
    if (!$head_parish_id || !$member_id || !$service_id) {
    ...
    $stmt = $conn->prepare("SELECT 
        ...
        payment_date,
        payment_status
    FROM sunday_service_payments 
    WHERE member_id = ? 
      AND service_id = ? 
      AND head_parish_id = ? 
    ORDER BY payment_date DESC");
    
    $stmt->bind_param("iii", $member_id, $service_id, $head_parish_id);
