...
$paymentReason = isset($_POST['description']) ? $conn->real_escape_string(trim($_POST['description'])) : null;
$paymentDate = isset($_POST['clientDateTime']) ? trim($_POST['clientDateTime']) : null;
$headParishId = isset($_POST['headParishId']) ? intval($_POST['headParishId']) : null;
...
$revenueStreamId = isset($_POST['offeringId']) ? intval($_POST['offeringId']) : null;
...
if (is_null($headParishId)) {
    echo json_encode(["success" => false, "message" => "headParishId is required"]);
    exit();
}

if (is_null($serviceDate)) {
    echo json_encode(["success" => false, "message" => "service_date is required"]);
    exit();
}

if (is_null($revenueStreamId)) {
    echo json_encode(["success" => false, "message" => "offeringId (revenueStreamId) is required"]);
    exit();
}
...
// Validate service_date format (ensure it's in Y-m-d format)
if (!strtotime($serviceDate)) {
    echo json_encode(["success" => false, "message" => "Invalid service date format"]);
    exit();
}
...
$paymentReason = "Malipo ya sadaka ya $revenueStreamName kutoka kwa $buyerName";
...
if ($response['status']) {
    // Insert into sunday_service_payments using the reusable function
    $paymentGateway->insertSundayServicePaymentData(
        $memberId,
        $headParishId,
        $response,
        $amount,
