...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($count > 0) {
        echo json_encode(["success" => false, "message" => "This phone number is already registered with an account."]);
        exit();
    }

    // Delete existing tokens for the member
    $delete_tokens_query = "DELETE FROM church_member_registration_tokens WHERE member_id = ?";
    $stmt = $conn->prepare($delete_tokens_query);
    $stmt->bind_param("i", $member_id);
    $stmt->execute();
    if ($stmt->error) {
        error_log("Failed to delete old tokens: " . $stmt->error);
        echo json_encode(["success" => false, "message" => "An error occurred while processing your registration."]);
        exit();
    }
    $stmt->close();
    ...
    $insert_query = "INSERT INTO church_member_registration_tokens (member_id, registration_token, status, created_at, token_expiry) 
                     VALUES (?, ?, 'unused', ?, ?)";
