...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    // Base query to fetch groups with description
    $sql = "SELECT group_id, group_name, IFNULL(description, 'No description provided') as description 
            FROM groups 
            WHERE head_parish_id = ?";

    // Apply search filter if provided
    if (!empty($searchQuery)) {
        $sql .= " AND group_name LIKE '%$searchQuery%'";
    }
    ...
    if ($stmt->execute()) {
        ...
        $countSql = "SELECT COUNT(*) as total FROM groups WHERE head_parish_id = ?";
        if (!empty($searchQuery)) {
            $countSql .= " AND group_name LIKE '%$searchQuery%'";
        }
        ...
        echo json_encode(["success" => false, "message" => "Failed to fetch groups: " . $stmt->error]);
    }
