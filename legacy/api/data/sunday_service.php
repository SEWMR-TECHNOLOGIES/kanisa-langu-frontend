...
try {
    // Retrieve 'head_parish_id' and 'service_date' from request (GET or POST)
    $headParishId = isset($_GET['head_parish_id']) ? $_GET['head_parish_id'] : null;
    $serviceDate = isset($_GET['service_date']) ? $_GET['service_date'] : null;

    // Query to fetch Sunday services with filtering
    $query = "
        SELECT 
            ss.service_id,
            ss.head_parish_id,
        ...
        WHERE (:head_parish_id IS NULL OR ss.head_parish_id = :head_parish_id)
        AND (:service_date IS NULL OR ss.service_date = :service_date)
    ";
    ...
    foreach ($services as $service) {
        ...
        $offerings = $offeringsStmt->fetchAll(PDO::FETCH_COLUMN);
