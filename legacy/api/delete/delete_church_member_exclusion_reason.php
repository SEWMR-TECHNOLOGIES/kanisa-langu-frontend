...
$stmt = $conn->prepare("SELECT reason FROM church_member_exclusion_reasons WHERE exclusion_reason_id = ? AND head_parish_id = ?");
...
$stmt = $conn->prepare("SELECT COUNT(*) AS usage_count FROM church_member_exclusions WHERE exclusion_reason_id = ?");
...
// Safe to delete
$stmt = $conn->prepare("DELETE FROM church_member_exclusion_reasons WHERE exclusion_reason_id = ?");
$stmt->bind_param("i", $reason_id);
