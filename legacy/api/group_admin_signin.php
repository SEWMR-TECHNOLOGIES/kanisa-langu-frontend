...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Query to join group_admins, groups, and head_parishes tables
    $sql = "
        SELECT ga.group_admin_id, ga.group_admin_fullname, ga.group_admin_email, 
               ga.group_admin_phone, ga.group_admin_password, ga.group_admin_role, 
               g.group_id, g.group_name, hp.head_parish_id, hp.head_parish_name
        FROM group_admins ga
        JOIN groups g ON ga.group_id = g.group_id
        JOIN head_parishes hp ON ga.head_parish_id = hp.head_parish_id
        WHERE ga.group_admin_email = ? OR ga.group_admin_phone = ?
    ";
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $user['group_admin_password'])) {
            // Store details in the session
            $_SESSION['group_admin_id'] = $user['group_admin_id'];
            $_SESSION['group_admin_email'] = $user['group_admin_email'];
            $_SESSION['group_admin_fullname'] = $user['group_admin_fullname'];
            $_SESSION['group_admin_phone'] = $user['group_admin_phone'];
            $_SESSION['group_admin_role'] = $user['group_admin_role'];
            $_SESSION['group_id'] = $user['group_id'];
            $_SESSION['group_name'] = $user['group_name'];
            $_SESSION['head_parish_id'] = $user['head_parish_id'];
            $_SESSION['head_parish_name'] = $user['head_parish_name']; // Store head parish name

            // Check if it's the first time logging in
            $firstTimeLogin = isAdminFirstLogin($admin_type, $user['group_admin_id']);
            $response = [
                "success" => true,
                "message" => "Login successful",
                "data" => [
                    "group_admin_id" => $user['group_admin_id'],
                    "group_admin_role" => $user['group_admin_role'],
                    "first_time_login" => $firstTimeLogin,
                    "head_parish_id" => $user['head_parish_id'], // Include head parish ID
                    "head_parish_name" => $user['head_parish_name'] // Include head parish name
                ]
            ];
            ...
            if ($firstTimeLogin) {
                ...
                saveAdminResetCode($user['group_admin_id'], $resetCode, $admin_type, $client_time);
                ...
                $emailSent = sendEmail($user['group_admin_email'], "Password Reset Code", $emailContent);
