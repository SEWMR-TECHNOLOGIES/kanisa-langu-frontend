...
<script>
  ...
  $(document).ready(function () {
    ...
    function loadHarambeeGroups(targetId, target, harambeeId, subParishId = null) {
      // Build the base URL
      let url = '../api/data/harambee_groups.php?target=' + target + '&harambee_id=' + harambeeId;
    
      // Append the sub_parish_id if provided
      if (subParishId) {
        url += '&sub_parish_id=' + subParishId;
      }
      $.ajax({
        type: 'GET',
        url: url,
        ...
        // Attach change events for both Harambee ID and Sub Parish ID
        $('#harambeeIdHP, #subParishIdHP').change(function () {
          const targetId = 'headParishGroupsTableBody'; // Target table body ID
          const target = 'head-parish'; // Target value for Head Parish
          const harambeeId = $('#harambeeIdHP').val(); // Get selected Harambee ID
          const subParishId = $('#subParishIdHP').val(); // Get selected Sub Parish ID
        
          if (harambeeId && subParishId) { // Only load if Harambee ID is selected
            loadHarambeeGroups(targetId, target, harambeeId, subParishId);
          } else {
            $('#' + targetId).html(''); // Clear the table if no Harambee ID
          }
        });
