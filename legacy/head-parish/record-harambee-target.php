        ...
        <div class="card">
          <div class="card-body">
            ...
            <ul class="nav nav-tabs" id="targetTabs" role="tablist">
              ...
              <li class="nav-item" role="presentation">
                <a class="nav-link" id="group-harambee-tab" data-bs-toggle="tab" href="#group-harambee" role="tab">Update Group Harambee Target</a>
              </li>
            </ul>
            ...
            <!-- Update Harambee Target Form -->
              <div class="tab-pane fade show" id="group-harambee" role="tabpanel">
                <form id="sharedHarambeeForm" autocomplete="off">
                    <div class="row">
                        <div class="col-lg-6">
                          <div class="mb-3">
                            <label for="targetTypeSelectionShared" class="form-label">Select Type</label>
                            <select class="form-select" id="targetTypeSelectionShared" name="target">
                              <option value="">Select Type</option>
                              <option value="head-parish">Head Parish</option>
                              <option value="sub-parish">Sub Parish</option>
                              <option value="community">Community</option>
                        ...
                        <div class="col-lg-6">
                            <div class="mb-3">
                              <label for="harambeeIdShared" class="form-label">Harambee</label>
                              <select class="form-select" id="harambeeIdShared" name="harambee_id">
                                <option value="">Select Harambee</option>
                                <!-- Options populated by AJAX -->
                              </select>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                          <div class="mb-3">
                            <label for="sharedHarambeeGroupId" class="form-label">Select Group</label>
                            <select class="form-select" id="sharedHarambeeGroupId" name="shared_harambee_group_id">
                        ...
                        <div class="col-lg-6">
                          <div class="mb-3">
                            <label for="newTargetAmount" class="form-label">New Target Amount</label>
                            <input type="number" class="form-control" id="newTargetAmount" name="new_target_amount" placeholder="New Target Amount" min="0">
                          </div>
                        </div>
                    </div>
                    <div class="mb-3">
...
$('#targetTypeSelectionShared').change(function () {
  const selectedType = $(this).val();
  loadHarambee('harambeeIdShared', `../api/data/head_parish_harambee?limit=all&target=${selectedType}`);
});
    ...
    // Function to load harambee
    function loadSharedHarambeeGroupNames(targetId, url) {
      $.ajax({
        type: 'GET',
        url: url,
        dataType: 'json',
    ...
    // Load member's target and contributions when a member is selected
   $('#harambeeIdShared, #targetTypeSelectionShared').change(function () { 
        const harambee_id = $('#harambeeIdShared').val();
        const target = $('#targetTypeSelectionShared').val();
        const url = `../api/data/get_shared_harambee_group_names?harambee_id=${harambee_id}&target=${target}`
        const targetId  = 'sharedHarambeeGroupId';
        // Check if all selections are made before making the API call
        if (harambee_id && target) {
            loadSharedHarambeeGroupNames(targetId, url)
        }
    });
    ...
    // Ignore if the form ID is sharedHarambeeForm
      if (formId === '#sharedHarambeeForm') {
    ...
    // Form submission logic
    function submitSharedHarambeeForm(formId) {
      const formData = $(formId).serialize();
    
      $.ajax({
        type: 'POST',
        url: '../api/records/update_shared_harambee_target', 
        data: formData,
        dataType: 'json',
        success: function (response) {
          let messageHtml = '';
    ...
    $('#sharedHarambeeForm').submit(function (e) {
      e.preventDefault();
      // Optionally show a loading indicator
      $('#responseMessage').html('<div class="response-message"><i class="fas fa-spinner fa-spin icon"></i> Sending Request...</div>');
      submitSharedHarambeeForm('#sharedHarambeeForm');
    });
