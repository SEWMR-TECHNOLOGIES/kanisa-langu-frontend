...
if ($admin_id) {
    try {
        $admin_id = decryptData($admin_id);
        if (empty($admin_id) || !preg_match('/^\d+$/', $admin_id)) {
            echo json_encode(["success" => false, "message" => "Invalid admin ID."]);
            exit;
        }
    } catch (Exception $e) {
        echo json_encode(["success" => false, "message" => "Error decrypting admin ID: " . $e->getMessage()]);
        exit;
    }
} else {
...
$admin_info = getHeadParishAdminDetailsById($conn, $head_parish_id, $admin_id);
...
$on_date_member_ids = getMemberIdsRecordedByAdmin($conn, $harambee_id, $contribution_date, $target, $admin_id);
...
foreach ($on_date_member_ids as $member_id) {
    ...
    $contribution_result = getTotalContributionsBetweenDates($conn, $harambee_id, $member_id, $contribution_date, $contribution_date, $target_table, $admin_id);
...
foreach ($on_date_member_details_array as $member) {
    ...
    if ($timestamp === null) {
        // You can decide whether to skip or group null timestamps separately
        continue;
    }
    ...
    if ($previous_time === null) {
        ...
        if ($diff_in_minutes <= 60) {
            // Within 1 hour of previous, same group
            $current_group[] = $member;
        } else {
            // More than 1 hour apart, close current group and start new
...
//         echo "- " . $member['name'] . " at " . $member['latest_local_timestamp'] . "\n";
//     }
// }
// exit();
        ...
        <div class="table-container">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="padding: 8px;"><strong>Jina la Karani:</strong> ' . $admin_info['full_name'] . '</td>
                        <td style="padding: 8px;text-align:right;"><strong>Nambari ya Simu:</strong> ' . $admin_info['phone'] . '</td>
                    </tr>
                </tbody>
            </table>
        </div>';  
   ...
   $counter = 1;
    foreach ($group as $member) {
       ...
       foreach ($member['payment_methods'] as $method) {
            ...
            $paymentMethodTotals[$method] += $member['amount_contributed_on_date'];
        ...
        $counter++; $attendedMemberCount++;
        
        
    }
    $grandTotal += $totalContributedOnDate;
...
$html .='<div class="table-container">
            <table style="width: 100%; border-collapse: collapse;">
                <tbody>
                    <tr>
                        <td style="padding: 8px;"><strong>Idadi ya Washarika:</strong> ' . $attendedMemberCount . '</td>
                        <td style="padding: 8px;text-align:right;"><strong>Jumla Kuu Taslimu:</strong> ' . number_format($grandTotal, 0) . '</td>
                    </tr>
                </tbody>
            </table>
        </div>';  
...
$filename = "{$admin_info['full_name']} Recorded Harambee Contributions on {$contribution_date_formatted}.pdf";
