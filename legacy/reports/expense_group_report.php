...
// Validate
if (!$management_level || !$head_parish_id || !$expense_group_id) {
...
$expense_summary = getExpenseGroupSummary(
    $conn,
    $management_level,
    $head_parish_id,
    $sub_parish_id,
...
$group_budget = (float)($expense_summary['annual_budget'] ?? 0);
$group_prev = (float)($expense_summary['total_expenses_before_date'] ?? 0);
$group_now = (float)($expense_summary['total_expenses'] ?? 0);
$group_total_expense = $group_prev + $group_now;
$group_balance = (float)($expense_summary['balance'] ?? ($group_budget - $group_total_expense));
...
<title>Expense Group Report</title>
<style>
  ...
  <div class="container">
    ...
    <div class="table-container">
      <table>
        ...
        <tbody>
          <tr>
            <td class="text-center">'.number_format($group_budget,0).'</td>
            <td class="text-center">'.number_format($group_prev,0).'</td>
            <td class="text-center">'.number_format($group_now,0).'</td>
            <td class="text-center">'.number_format($group_total_expense,0).'</td>
      ...
      <table>
        <thead>
          ...
          <tr>
            ...
            <th style="width:30%;text-align:center;" rowspan="2">JINA LA MATUMIZI</th>
...
foreach ($expenses as $exp) {
    $exp_name = htmlspecialchars($exp['expense_name'] ?? ($exp['name'] ?? '-'));
    $exp_budget = (float)($exp['budgeted_amount'] ?? $exp['budget'] ?? 0);
    $exp_prev = (float)($exp['total_expenses_before_date'] ?? 0);
    $exp_now = (float)($exp['total_expenses'] ?? 0);
    ...
    <tr>
        ...
        <td style="text-align:left; padding-left:8px;">'.$exp_name.'</td>
        <td class="text-right">'.number_format($exp_budget,0).'</td>
        <td class="text-right">'.number_format($exp_prev,0).'</td>
        <td class="text-right">'.number_format($exp_now,0).'</td>
        <td class="text-right">'.number_format($exp_total,0).'</td>
        <td class="text-right '.($exp_balance < 0 ? 'amount-neg' : '').'">'.number_format($exp_balance,0).'</td>
    </tr>';
}
...
$filename = preg_replace('/\s+/', '_', strtoupper($display_name)) . "_EXPENSE_GROUP_REPORT" . date('Ymd_His') . ".pdf";
