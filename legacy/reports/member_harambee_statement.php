...
if ($member_id) {
    ...
    // Calculate balance and title logic
    if ($target_amount != 0 && $total_contribution > $target_amount) {
        // Contribution is greater than target → ZIADA
        $title = 'ZIADA';
        $balance = abs($total_contribution - $target_amount); // positive difference
    } else {
        // Normal balance (SALIO)
        $title = 'SALIO';
