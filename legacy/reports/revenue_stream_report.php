...
$revenue_stream_id  = isset($_GET['revenue_stream_id']) ? intval($_GET['revenue_stream_id']) : null; // NEW
...
// Validate
if (!$management_level || !$head_parish_id || !$revenue_group_id || !$revenue_stream_id) {
...
// Get revenue stream summary (replaces expense summary)
$revenue_summary = getRevenueStreamSummary(
    $conn,
    $management_level,
    $head_parish_id,
    $sub_parish_id,
...
if ($revenue_summary === null || $revenue_summary === false) {
    echo json_encode(["success" => false, "message" => "No revenue data found"]);
    exit();
}

// Encode the response as JSON and log it
// error_log("getExpenseNameSummary response: " . json_encode($revenue_summary, JSON_PRETTY_PRINT));


$group_name   = $revenue_summary['revenue_group_name'] ?? 'Unknown';
$revenue_name = strtoupper($revenue_summary['revenue_stream_name'] ?? 'Unknown');
$rev_name_id  = (int)($revenue_summary['revenue_stream_id'] ?? 0);
$rev_budget   = (float)($revenue_summary['total_target'] ?? 0);
$annual_target= $rev_budget;
$rev_prev     = (float)($revenue_summary['total_revenues_before_date'] ?? 0);
$rev_now      = (float)($revenue_summary['total_revenues'] ?? 0);
$rev_total    = $rev_prev + $rev_now;
$rev_balance  = (float)($revenue_summary['balance'] ?? ($rev_budget - $rev_total));
...
<title>Revenue Stream Report</title>
<style>
  ...
  <div class="container">
    <div class="header">
      ...
      <h3 style="color:#2c3e50;">
         TAARIFA YA '.htmlspecialchars($group_name).' MAPATO YA '.htmlspecialchars($revenue_name).' KUTOKA '.$start_date_disp.' HADI '.$end_date_disp.''.($management_level !== 'head-parish' ? ' '.$display_name : '').
      '</h3>
    </div>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th colspan="5" style="width:96%;text-align:center; font-weight:700;">MUHTASARI WA '.htmlspecialchars($group_name).' MAPATO YA '.htmlspecialchars($revenue_name).'</th>
          </tr>
          <tr>
        ...
        <tbody>
          <tr>
            <td class="text-center">'.number_format($rev_budget,0).'</td>
            <td class="text-center">'.number_format($rev_prev,0).'</td>
            <td class="text-center">'.number_format($rev_now,0).'</td>
            <td class="text-center">'.number_format($rev_total,0).'</td>
            <td class="text-center">'.number_format($rev_balance,0).'</td>
          </tr>
        </tbody>
      </table>
    </div>

  <!-- Table B: Revenue breakdown -->
    <div class="table-container">
      <table>
        <thead>
          <tr>
            <th class="text-center" style="width:5%;">B:</th>
            <th colspan="7" style="text-align:center; font-weight:700;width:95%;">MAPATO YA '.htmlspecialchars($revenue_name).'</th>
          </tr>
          <tr>
            ...
            <th style="width:14%;text-align:center;" rowspan="2">MALENGO</th>
    ...
    $count = 1;
    $running_balance = $rev_budget - $rev_prev;

    foreach ($revenues as $rev) {
        $date = date('d-m-Y', strtotime($rev['date'] ?? ''));
        $amount_now = (float)($rev['amount'] ?? 0);
        $total = $rev_prev + $amount_now;
        $running_balance -= $amount_now;
        $revenue_description = $revenue_name;

        $html .= '
        <tr>
            <td class="small" style="text-align:center">'.$count++.'</td>
            <td style="text-align:left; padding-left:8px;">'.$date.'</td>
            <td style="text-align:left; padding-left:8px;">'.htmlspecialchars($revenue_description).'</td>
            <td class="text-right">'.number_format($rev_budget,0).'</td>
...
$filename = preg_replace('/\s+/', '_', strtoupper($display_name)) . "_REVENUE_STREAM_REPORT_" . date('Ymd_His') . ".pdf";
