...
class PaymentGateway
{
    ...
    private const VENDOR_ID = 'TILL61137836';
    private const CALLBACK_URL = 'https://kanisalangu.sewmrtechnologies.com/payments/';
    ...
    public function requestPayment($phoneNumber, $amount, $transactionDesc, $buyerName, $buyerEmail)
    {
        ...
        // Construct data to post to the Selcom order/payment script
        $postData = [
            'amount' => $amount,
            ...
            'buyer_remarks' => $transactionDesc,
            'merchant_remarks' => $transactionDesc,
            'no_of_items' => 1
        ];
    
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://payment-gateway.sewmrtechnologies.com/selcom/make-payment.php'); 
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        ...
        if ($httpCode !== 200) {
            return ['status' => false, 'message' => 'Selcom order request failed'];
        }
        ...
        return [
            ...
            'message' => $responseArray['order_response']['message'] ?? 'Payment processed successfully'
        ];
    ...
    public function checkTransactionStatus($checkoutRequestId)
    {
        if (empty($checkoutRequestId)) {
            return ['status' => false, 'message' => 'Missing order ID'];
        }
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://payment-gateway.sewmrtechnologies.com/selcom/check-payment-status.php');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($postData));
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
        ...
        if ($httpCode !== 200 || !$response) {
            return [
                'status' => false,
                'message' => 'Selcom status request failed: ' . $curlError
            ];
        }
        ...
        // Now check if payment_status exists in data[0]
        if (!empty($responseArray['payment_status'])) {
            $status = strtoupper($responseArray['payment_status']);
    
            if ($status === 'COMPLETED') {
                return ['status' => true, 'message' => 'Payment Successful', 'payment_status' => $status];
            } elseif ($status === 'PENDING' || $status === 'INPROGRESS') {
                return ['status' => false, 'message' => 'Payment Still Pending', 'payment_status' => $status];
            } else {
                return ['status' => false, 'message' => 'Payment Failed: ' . $status, 'payment_status' => $status];
            }
        }
    ...
    public function insertHarambeePaymentData($memberId, $harambeeId, $headParishId, $responseData, $amount, $paymentReason, $paymentDate, $target = 'head-parish', $paymentStatus = 'Pending')
    {
        ...
        $checkoutRequestID = $responseData['response']['order_id'] ?? null;
        $transactionReference = $responseData['response']['wallet_payment_response']['transid'] ?? null;
        $merchantRequestID = $responseData['response']['order_id'] ?? null;
