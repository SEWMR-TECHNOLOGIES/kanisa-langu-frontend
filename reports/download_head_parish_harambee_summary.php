...
// Validate target
$valid_targets = ['head-parish', 'sub-parish', 'community', 'groups', 'group'];
...
$all_members_ids = getHarambeeMemberIdsByContributionDate($conn, $harambee_id, $target);
...
foreach ($on_date_member_ids as $member_id) {
    ...
    if ($member['group_name'] != null) {
        ...
        // This is an individual — log their member_id
        error_log("Individual member ID: " . $member_id);
...
$sub_parish_distributed_amount = $harambee_details['amount'];
...
foreach ($all_members_ids as $member_id) {
    // Get the member details
    $member = getSingleMemberHarambeeDetails($conn, $member_id, $harambee_id, $target);
...
$total_harambee_contribution = getTotalContributionFromArray($all_member_details_array);
            ...
            <table>
                <thead>
                    <tr style="background-color:#fff;font-weight:bold;text-align:center;">
                        <td colspan="2">
                            TAARIFA YA MAPATO YA HARAMBEE YA ' . strtoupper(htmlspecialchars($harambee_details['description'])) . ' KUTOKA <span class="date">' . 
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td style="text-align: center; font-weight: bold;color:blue;">' . $grand_member_count . '</td>
                    </tr>
                </tbody>
            </table>
        </div>';
        ...
        <table border="1" cellpadding="5" cellspacing="0">
            <thead>
                <tr>
                    ...
                    <th>IDADI</th>
                    <th>TASLIMU</th>
                    <th style="border-left:2px solid #000;">JUMUIYA</th>
                    <th>IDADI</th>
                    <th>TASLIMU</th>
                </tr>
            </thead>
...
for ($i = 0; $i < $maxRows; $i++) {
    ...
    if (isset($columns[1][$i])) {
        // $html .= '<td style="font-weight: bold; text-align: center;">' . $serialNumber++ . '</td>'; // Bold serial number
        $html .= '<td style="border-left:2px solid #000;">' . $columns[1][$i]['name'] . '</td>';
        ...
        $html .= '<td style="border-left:2px solid #000;"></td><td></td><td></td>'; // Empty cells if no data
    }
...
$filename = " Harambee Contribution from " . $from_date_formatted . " to " . $to_date_formatted . ".pdf";
