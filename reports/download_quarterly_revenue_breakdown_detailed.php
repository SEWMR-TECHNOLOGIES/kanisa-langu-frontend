...
switch ($quarter) {
    case 'Q1':
        // Q1's previous quarter is Q4 of the previous year
        $previousQuarterEnd = (PREVIOUS_YEAR) . '-' . Q4_END; // Previous quarter is Q4 of the previous year
        break;
    case 'Q2':
        // Q2's previous quarter is Q1 of the current year
        $previousQuarterEnd = CURRENT_YEAR . '-' . Q1_END; // Previous quarter is Q1
        break;
    case 'Q3':
        // Q3's previous quarter is Q2 of the current year
        $previousQuarterEnd = CURRENT_YEAR . '-' . Q2_END; // Previous quarter is Q2
        break;
    case 'Q4':
        // Q4's previous quarter is Q3 of the current year
        $previousQuarterEnd = CURRENT_YEAR . '-' . Q3_END; // Previous quarter is Q3
        break;
}
...
$currentYearRevenueData = getHeadParishRevenueBySubParishQuarterly($head_parish_id, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END , $account_id, $conn);
$total_revenues = calculateTotalRevenue($currentYearRevenueData);
$envelopeData = getEnvelopeContributionsBySubParishQuarterly($head_parish_id, CURRENT_YEAR, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END,$conn);
$bankAccountsData = getAccountWiseFinancialSummaryQuarterly($head_parish_id, $previousQuarterEnd, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $conn);
$subParishesRevenuesData = getSubParishYearlyFinancialsQuarterly($head_parish_id, $previousQuarterEnd, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $conn);
$overBudgetExpenseData = getOverBudgetExpenseNamesQuarterly($head_parish_id, CURRENT_YEAR, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END,$conn);
$debtsData = getUnpaidDebtsByQuarter($head_parish_id, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $conn);
...
if (empty($currentYearRevenueData) || !array_filter($currentYearRevenueData, fn($rev) => !empty($rev['items']))) {
    ...
    for ($start = 0; $start < $maxItems; $start += $itemsPerBatch) {
        ...
        foreach ($columnTotals as $columnTotal) {
            ...
            $uwianoValue = ($calculatedUwianoValues[$columnTotal['id']])/4;
        ...
        foreach ($columnTotals as $columnTotal) {
            ...
            $rawUwiano = $calculatedUwianoValues[$columnTotal['id']] ?? 0;
            $uwianoValue = $rawUwiano / 4;
        
            if ($uwianoValue > 0) {
                $percentageValue = (($uwianoValue - $columnTotal['total']) / $uwianoValue) * 100;
            } else {
                $percentageValue = 0; // or null / '-' depending on report rules
            }
    ...
    <table border="1" cellpadding="5" cellspacing="0">
        <thead> 
            ...
            <tr>
                ...
                <th style="width:15.4%">'.$quarter.' / WEEKLY AVERAGE</th>
            </tr>
        </thead>
        <tbody>';
            ...
            foreach ($envelopeData as $data) {
                ...
                $weeklyAverage = ($data['total_envelopes'] != 0) ? $totalContribution / (13 * $data['total_envelopes']) : 0;
    ...
    <table border="1" cellpadding="5" cellspacing="0">
        <thead> 
            ...
            <tr>
                ...
                <th style="width:15.4%">'.$quarter.' REVENUE</th>
    ...
    <table border="1" cellpadding="5" cellspacing="0">
        <thead> 
            <tr>
                ...
                <th colspan="6" class="text-left header-row-bg">'.$quarter.' OVER BUDGET EXPENSES</th>
            </tr>
            <tr>
                <th style="width:3%">No.</th>
                <th style="width:20%">EXPENSE NAME</th>
                <th style="width:15.4%">'.$quarter.' BUDGET</th>
                <th style="width:15.4%">'.$quarter.' EXPENSE</th>
                <th style="width:15.4%">'.$quarter.' BALANCE</th>
...
$filename = CURRENT_YEAR." ". $quarter." Quarterly Revenue Breakdown Statement.pdf";
