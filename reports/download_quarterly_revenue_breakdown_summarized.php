...
// Get the quarter word from the quarter name
$quarterWord = isset($quarterNames[$quarter]) ? $quarterNames[$quarter] : '';

// Convert date strings to the desired format
$startDate = DateTime::createFromFormat('Y-m-d', CURRENT_YEAR_QUARTER_START)->format('M d Y');
$endDate = DateTime::createFromFormat('Y-m-d', CURRENT_YEAR_QUARTER_END)->format('M d Y');
...
// Fetch attendance data for the selected quarter in the current and previous year
$currentQuarterAttendance = getQuarterlyAverageAttendance(CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $head_parish_id, $conn);
$previousQuarterAttendance = getQuarterlyAverageAttendance(PREVIOUS_YEAR_QUARTER_START, PREVIOUS_YEAR_QUARTER_END, $head_parish_id, $conn);

// Store attendance in separate variables
$currentQuarterAdultAttendance = $currentQuarterAttendance['average_adult_attendance'];
$currentQuarterChildrenAttendance = $currentQuarterAttendance['average_children_attendance'];

$previousQuarterAdultAttendance = $previousQuarterAttendance['average_adult_attendance'];
$previousQuarterChildrenAttendance = $previousQuarterAttendance['average_children_attendance'];

// Fetch the attendance benchmark for adults and children
$attendanceBenchmarks = getAttendanceBenchmark($head_parish_id, $conn);
$adultBenchmark = $attendanceBenchmarks['adult_reading'];
$childrenBenchmark = $attendanceBenchmarks['child_reading'];

// Calculate the percentage of previous quarter attendance compared to benchmark
$previousQuarterAdultPercentage = ($adultBenchmark > 0) ? round(($previousQuarterAdultAttendance / $adultBenchmark) * 100, 0) : 0;
$previousQuarterChildrenPercentage = ($childrenBenchmark > 0) ? round(($previousQuarterChildrenAttendance / $childrenBenchmark) * 100, 0) : 0;

// Calculate the percentage of current quarter attendance compared to benchmark
$currentQuarterAdultPercentage = ($adultBenchmark > 0) ? round(($currentQuarterAdultAttendance / $adultBenchmark) * 100, 0) : 0;
$currentQuarterChildrenPercentage = ($childrenBenchmark > 0) ? round(($currentQuarterChildrenAttendance / $childrenBenchmark) * 100, 0) : 0;

// Calculate the difference between current quarter attendance and benchmark
$currentQuarterAdultDifference = $currentQuarterAdultAttendance - $adultBenchmark;
$currentQuarterChildrenDifference = $currentQuarterChildrenAttendance - $childrenBenchmark;


// Fetch revenue group financials for both years
$previousYearData = getHeadParishQuarterlyRevenueGroupFinancials($head_parish_id, PREVIOUS_YEAR, PREVIOUS_YEAR_QUARTER_START, PREVIOUS_YEAR_QUARTER_END, $conn);
$currentYearData = getHeadParishQuarterlyRevenueGroupFinancials($head_parish_id, CURRENT_YEAR, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $conn);
    ...
    <div class="container">
        <div class="header" style="text-align: center; margin-bottom: 5px;">
            ...
            <h1 style="font-size: 10px; color: #2c3e50; margin: 2px 0;">
                TAARIFA YA MAPATO, MATUMIZI NA MALI ZA USHARIKA ROBO YA ' . strtoupper($quarterWord) . ' YA MWAKA
                <span class="date">' . strtoupper($startDate) . ' - ' . strtoupper($endDate) . '</span>
            </h1>
        </div>
    </div>
    ...
    <table border="1" cellpadding="5" cellspacing="0">
        <thead> 
            ...
            <tr>
                ...
                <th colspan="2" style="width:20%">'.$quarter.' - '.PREVIOUS_YEAR.'</th>
                <th colspan="2" style="width:20%">'.$quarter.' - '.CURRENT_YEAR.'</th>
                <th colspan="2" style="width:20%">MAFANIKIO</th>
                <th style="width:6%">UWIANO</th>
                <th rowspan="2" style="width:11%">TOFAUTI '.$quarter. ' - ' .CURRENT_YEAR.'</th>
            </tr>
            <tr>
                ...
                <th style="width:10%">BAJETI</th>
                <th style="width:10%">HALISI</th>
                <th style="width:10%">'.$quarter.' - '.PREVIOUS_YEAR.'</th>
                <th style="width:10%">'.$quarter.' - '.CURRENT_YEAR.'</th>
                <th style="width:6%">'.$quarter.' - '.CURRENT_YEAR.'</th>
            </tr>
        </thead>
        <tbody>
        <tr class="text-center">
            ...
            <td class="column-bg-color">'.$previousQuarterAdultAttendance.'</td>
            <td>'.$adultBenchmark.'</td>
            <td>'.$currentQuarterAdultAttendance.'</td>
            <td class="column-bg-color">'.$previousQuarterAdultPercentage.'%</td>
            <td>'.$currentQuarterAdultPercentage.'%</td>
            <td>-</td>
            <td class="text-center" style="color:'.($currentQuarterAdultDifference < 0 ? 'red' : 'black').'">
                '.($currentQuarterAdultDifference).'
            </td>
        </tr>
        <tr class="text-center">
            <td class="text-center">2</td>
            <td class="text-left">Watoto</td>
            <td class="column-bg-color">'.$childrenBenchmark.'</td>
            <td class="column-bg-color">'.$previousQuarterChildrenAttendance.'</td>
            <td>'.$childrenBenchmark.'</td>
            <td>'.$currentQuarterChildrenAttendance.'</td>
            <td class="column-bg-color">'.$previousQuarterChildrenPercentage.'%</td>
            <td>'.$currentQuarterChildrenPercentage.'%</td>
            <td>-</td>
            <td class="text-center" style="color:'.($currentQuarterChildrenDifference < 0 ? 'red' : 'black').'">
                '.($currentQuarterChildrenDifference).'
            </td>
        </tr>';
...
// Fetch revenue group financials for both years
$previousYearData = getRevenueDataByHeadParishQuarterly($head_parish_id, PREVIOUS_YEAR, PREVIOUS_YEAR_QUARTER_START, PREVIOUS_YEAR_QUARTER_END, $conn);
$currentYearData = getRevenueDataByHeadParishQuarterly($head_parish_id, CURRENT_YEAR, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $conn);
...
$previousYearData = getExpenseDataByHeadParishQuarterly($head_parish_id, PREVIOUS_YEAR, PREVIOUS_YEAR_QUARTER_START, PREVIOUS_YEAR_QUARTER_END, $conn);
$currentYearData = getExpenseDataByHeadParishQuarterly($head_parish_id, CURRENT_YEAR, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $conn);
...
$assetsData = getHeadParishAssetsQuarterly($head_parish_id, CURRENT_YEAR_QUARTER_START, CURRENT_YEAR_QUARTER_END, $conn);
