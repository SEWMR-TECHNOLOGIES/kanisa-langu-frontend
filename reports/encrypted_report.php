...
if ($member_id) {
    ...
    if ($result->num_rows > 0) {
                    ...
                    <table class="table-auto w-full border-collapse">
                        <thead>
                            <tr>
                                ...
                                <th class="summary-header">SALIO HADI SASA</th>
        ...
        $tempFilePath = tempnam(sys_get_temp_dir(), 'temp_pdf_') . '.pdf';
        ...
        // Import the PDF content into TCPDF
        $pageCount = $tcpdf->setSourceFile($tempFilePath);
        ...
        $certificatePath = 'file://' . $_SERVER['DOCUMENT_ROOT'] . '/libraries/certificate.pem';
        $privateKeyPath = 'file://' . $_SERVER['DOCUMENT_ROOT'] . '/libraries/private_key.pem';
        ...
        $tcpdf->setSignature($certificatePath, $privateKeyPath, 'tcpdfdemo', '', 2, $info);


        
        // Place the signature on the PDF (coordinates can be adjusted)
        $tcpdf->setSignatureAppearance(180, 20, 30, 50);
        
        // Generate a dynamic filename
        $filename = str_replace(' ', '_', $sanitized_name) . "_member_envelope_report_signed.pdf"; // Add a suffix for signed report
        
        // Output the signed PDF with the dynamic filename
        $tcpdf->Output($filename, 'I'); // Stream to the browser directly
