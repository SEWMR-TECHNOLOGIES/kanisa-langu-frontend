...
if ($community_id) {
    ...
    // Handle case where sub_parish ID is not provided (optional, depending on your business logic)
    echo json_encode(["success" => false, "message" => "Valid community is needed."]);
    exit;
}
...
foreach ($on_date_member_ids as $member_id) {
    ...
    if ($community_id && $member['community_id'] != $community_id) {
                ...
                <thead>
                    <tr style="background-color:#fff;font-weight:bold;text-align:center;">
                        <td colspan="2">
                            ...
                            getSubParishName($sub_parish_id, $conn) . ' JUMUIYA YA '.getCommunityName($community_id, $conn).' KUTOKA <span class="date">' . 
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        ...
                        <td style="text-align: center; font-weight: bold;">' . number_format(max($sub_parish_distributed_amount - $total_harambee_contribution, 0), 2) . '</td>
...
$community_name = getCommunityName($community_id, $conn);

// Construct the filename with sub-parish name and date range
$filename = $community_name . " Harambee Contribution from " . $from_date_formatted . " to " . $to_date_formatted . ".pdf";
