...
export default function AdminLayout({ navigation, levelLabel, basePath }: AdminLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-admin-bg">
      ...
      <div className="lg:ml-[280px] min-h-screen flex flex-col min-w-0 overflow-x-hidden">
        <header className="sticky top-0 z-30 bg-admin-header border-b border-admin-border">
          <div className="flex items-center justify-between px-4 lg:px-8 h-16">
            <div className="flex items-center gap-4 min-w-0">
              <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-lg hover:bg-admin-surface-hover text-admin-text flex-shrink-0">
                <Menu className="w-5 h-5" />
              </button>
              <div className="hidden sm:flex items-center gap-1.5 text-[12px] min-w-0 overflow-hidden">
